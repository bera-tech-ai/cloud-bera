...
module.exports = {
    commands:    ['profile', 'myprofile', 'wainfo'],
    description: 'View a WhatsApp user profile info',
    permission:  'public',
    group:       true,
    private:     true,
    run: async (sock, message, args, ctx) => {
        const { sender, isGroup, jid, reply, contextInfo } = ctx;
        const chatId   = isGroup ? jid : sender;
...
_Powered by Bera AI_`;
            ...
            await reply(`❌ Profile fetch failed: ${e.message}`);
        }
    }
};
