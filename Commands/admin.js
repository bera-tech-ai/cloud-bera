...
Bera AI GitHub commands will now use this account.`)
    ...
    if (command === 'vercel' || command === 'vdeploy' || command === 'deployvercl') {
        ...
        if (!text) return reply(
            `❌ Usage: ${prefix}vercel <github-repo-url> [project-name]\n\n` +
            `Example: ${prefix}vercel https://github.com/bera-tech-ai/my-app\n\n` +
            `Or: ${prefix}vercel https://github.com/bera-tech-ai/my-app my-project-name`
        )
        const parts = text.trim().split(/\s+/)
        const repoUrl = parts[0]
        const projectName = (parts[1] || repoUrl.split('/').pop().replace(/\.git$/, '')).toLowerCase().replace(/[^a-z0-9-]/g, '-')
        if (!/^https?:\/\/github\.com\//i.test(repoUrl)) return reply(`❌ Provide a GitHub URL. Example: https://github.com/owner/repo`)

        await react('🚀')
        await reply(`🚀 Deploying *${projectName}* to Vercel...\n⏳ This takes 30-120 seconds.`)
        ...
        try {
            ...
            let projectId = null
            try {
                const projRes = await ax.post('https://api.vercel.com/v9/projects', {
                    name: projectName,
                    gitRepository: { type: 'github', repo: `${ghOwner}/${ghRepo}` }
                }, { headers, timeout: 20000 })
                projectId = projRes.data?.id
            } catch (e) {
                // Project may already exist
                if (e.response?.status === 409) {
                    const existing = await ax.get(`https://api.vercel.com/v9/projects/${projectName}`, { headers, timeout: 10000 })
            ...
            if (!projectId) throw new Error('Could not create or find Vercel project.')

            // Trigger a deployment
            const depRes = await ax.post(`https://api.vercel.com/v13/deployments`, {
                name: projectName,
                gitSource: { type: 'github', repoId: null, ref: 'main', org: ghOwner, repo: ghRepo }
            }, { headers, timeout: 30000 })

            const dep = depRes.data
            const depId  = dep?.id || dep?.uid
            const depUrl = dep?.url ? `https://${dep.url}` : `https://${projectName}.vercel.app`

            await react('✅')
            return reply(
                `╭══〘 *🚀 VERCEL DEPLOYED* 〙═⊷\n` +
                `┃❍ *Project:* ${projectName}\n` +
                `┃❍ *Status:* ${dep?.readyState || 'BUILDING'}\n` +
                `┃❍ *URL:* ${depUrl}\n` +
                `┃❍ *Repo:* ${ghOwner}/${ghRepo}\n` +
                `┃\n` +
                `┃ _Build takes 1-3 min. Check Vercel dashboard for progress._\n` +
                `╰══════════════════⊷`
            )
        } catch (e) {
            await react('❌')
            return reply(`❌ *Vercel deploy failed:*\n${e.response?.data?.error?.message || e.message}`)
        }
    }
    ...
    if (command === 'vercellist' || command === 'vprojects') {
        ...
        try {
            ...
            if (!projects.length) return reply(`📭 No Vercel projects found.`)
            const lines = projects.slice(0, 15).map((p, i) =>
                `${i + 1}. *${p.name}* — ${p.latestDeployments?.[0]?.readyState || 'no deploys'}`
            ).join('\n')
            return reply(`╭══〘 *🚀 VERCEL PROJECTS* 〙═⊷\n┃\n${lines}\n╰══════════════════⊷`)
        } catch (e) {
            return reply(`❌ ${e.response?.data?.error?.message || e.message}`)
        }
    }
